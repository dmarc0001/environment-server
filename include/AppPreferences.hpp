...
namespace Prefs
{
  ...
  constexpr const char *WEB_PATH{"/www"};                         //! virtual path wegserver
  ...
  constexpr uint32_t LED_STRIPE_COUNT = 1U;                       //! count of rgb-led
  ...
  constexpr gpio_num_t DHT_SENSOR_GPIO = GPIO_NUM_8;              //! port for sensor humidy and temperature
  constexpr gpio_num_t TEMPERATURE_SENSOR_GPIO = GPIO_NUM_14;     //! port fpr ds18b20 temperature sensor(es)
  ...
  constexpr int MEASURE_SCAN_SENMSOR_INTERVAL = 62; //! scan for new sensors
