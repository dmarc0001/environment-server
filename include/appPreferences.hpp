...
namespace Prefs
{
...
#endif
#endif
  constexpr const uint8_t LOGID{ 0 };                                                //! logId for elog
  ...
  constexpr const char *WEB_SITE_PATH{ "/www/" };                                    //! virtual path webserver data
  ...
  constexpr const char *NTP_POOL_EUROPE_NAME{ "europe.pool.ntp.org" };  //! ntp pool for time
  constexpr const char *NTP_POOL_GERMANY_NAME{ "de.pool.ntp.org" };     //! ntp pool for time
